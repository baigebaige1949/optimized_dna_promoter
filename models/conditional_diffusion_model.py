...
class ConditionalUNetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time_emb: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        """
        # --- channel align before residual conv (auto 1x1) ---
        if isinstance(self.residual, nn.Conv1d) and x.size(1) != self.residual.in_channels:
            if (not hasattr(self, 'input_align')
                or getattr(self.input_align, 'in_channels', None) != x.size(1)
                or getattr(self.input_align, 'out_channels', None) != self.residual.in_channels):
                self.input_align = nn.Conv1d(x.size(1), self.residual.in_channels, kernel_size=1, bias=False).to(x.device)
            x = self.input_align(x)
        # --- channel align before residual conv (auto 1x1, guarded) ---
...
class ConditionalDiffusionModel(nn.Module):
    ...
    def predict_noise(
        ...
        """预测噪声"""
        pred = self.unet(x_t, t, conditions)
        # --- length align: ensure pred L matches x_t L ---
        if pred.size(-1) != x_t.size(-1):
            pred = F.interpolate(pred, size=x_t.size(-1), mode='nearest')
        # --- (optional) channel align: pred C -> x_t C ---
        if pred.size(1) != x_t.size(1):
            import torch.nn as nn
            if (not hasattr(self, 'noise_align')
                or getattr(self.noise_align, 'in_channels', None) != pred.size(1)
                or getattr(self.noise_align, 'out_channels', None) != x_t.size(1)):
                self.noise_align = nn.Conv1d(pred.size(1), x_t.size(1), kernel_size=1, bias=False).to(pred.device)
            pred = self.noise_align(pred)
        return pred
    def p_losses(
        self,
